...
public class FaceFilterCameraManager {
    ...
    private void openCamera() {
        ...
        try {
            mCameraManager.openCamera(mCameraID, mStateCallback, mCameraBackgroundHandler);
    ...
    public void onCameraOpenResult(CameraDevice camera) {
        try {
            ...
            mCameraDevice.createCaptureSession(Arrays.asList(mCameraOutputSurface), callback, null);
        } catch (InterruptedException e) {
            throw new RuntimeException("Interrupted while trying to lock.", e);
        } catch (CameraAccessException e) {
